...
//-- ATRIBUTOS -----------------------------------------------------------------
	...
	private static Gift directGift;
...
//-- JUEGO ---------------------------------------------------------------------	
	...
	public void shoot(int pos) {
		...
		if (board.getBoxes()[pos] instanceof BoxSpecial) {
			...
			Game.directGift=getRandomGift();
			directGift.setUnits(1);
			selectedGifts.add(directGift);
		}
		this.points += board.getBoxes()[pos].getPoints();
	}
	
	/**
	 * Método que devuelve el regalo directo
	 * 
	 * @return directGift, regalo directo
	 */
...
//-- REGALOS -------------------------------------------------------------------
	...
	/**
	 * Método que consigue un regalo aleatorio, no perteneciente a la sección de viajes
	 * 
	 * @return randomGift, regalo aleatorio
	 */
